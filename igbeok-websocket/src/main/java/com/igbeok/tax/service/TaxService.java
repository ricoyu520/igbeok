package com.igbeok.tax.service;
...
public class TaxService {
	...
	public static void main(String[] args) {
		...
		baseLine = income;
		if (args.length > 1)
			baseLine = Float.parseFloat(args[1]);

		TaxService tax = new TaxService();

		float[] taxs = tax.caculateTax(income, baseLine, socialTaxRatePersonal);
	...
	public float[] caculateTax(float income, float baseLine, float socialTaxRate) {
		// 这个是交掉的公积金,如果超过了最高缴费基数, 则按最高缴费基数来计算
		float reservedFound = (baseLine > RESERVED_FOUND_TOP ? RESERVED_FOUND_TOP : baseLine) * socialTaxRate;
